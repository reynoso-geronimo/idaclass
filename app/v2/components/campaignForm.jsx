...
const CampaignForm = ({ nombre, short = false }) => {
  ...
  return (
    <div className=" container mb-12 p-4 border-2 border-border bg-gray-100 rounded-3xl shadow-lg z-10 max-w-sm">
      <Dialog open={open}>
        <DialogContent>
          <DialogHeader>
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className=" flex flex-col justify-evenly gap-4">
          {!short && (
            <h1 className="font-bold text-lg text-idaclass">
              Comienza ahora, <br /> aprovecha el descuento
            </h1>
          )}
          <FormField
          ...
          <Button
            type="submit"
            className="w-full rounded-full bg-green-500 hover:bg-green-400"
