...
/** Ficha individual: foto (o iniciales), bio, bullets, tags y stats. */
function SpeakerCard({ kicker, name, role, bio, bullets, tags, stats, photo }) {
  if (!hasContent(name)) return null

  return (
    <div className="grid overflow-hidden rounded-md border border-le-line bg-le-graphite md:grid-cols-[360px_1fr]">
      {/* Foto o iniciales */}
...
/**
 * Sección de orador/es. Dos formas de usarla:
 *   - Uno solo:  { type: 'speaker', name, role, bio, ... }            (props sueltas)
 *   - Varios:    { type: 'speaker', items: [{ name, ... }, { ... }] } (una card por item)
 * Si se pasan ambas, `items` gana. Sin ningún `name` la sección no se renderiza.
 */
export default function Speaker({ eyebrow, title, intro, items, ...single }) {
  const list = (hasContent(items) ? items : [single]).filter((s) => hasContent(s?.name))
