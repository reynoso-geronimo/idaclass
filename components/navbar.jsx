...
const NavBar = ({ cursosFormacion, cursosEspecializacion }) => {
  ...
  if (pathname.includes("v2") || pathname.includes("v3") || pathname.includes("Transforma-Tu-Pasion-en-Profesion-de-Impacto") || pathname.includes("landingfitnessnegocios") || pathname.includes("masterclass") || pathname.includes("eventos")) return null;
  return (
    <header className="sticky top-0 z-40 bg-black text-white">
      ...
      <nav className="w-full flex justify-between items-center h-16 container max-sm:px-1 font-semibold">
        <Sheet>
          ...
          <SheetContent side={"left"} className="w-72 h-[100svh] flex flex-col gap-4 text-base font-semibold">
            <SheetClose asChild>
              <Link className={`-translate-x-3.5 flex items-center ${pathname === "/" && "text-idaclass4"} `} href="/">
            ...
            <Separator />
            <SheetClose asChild>
              <Link className={`-translate-x-3.5 flex items-center `} href="https://campus-idaclassacademy.com/">
                <Minus className="rotate-90 -left-3 scale-x-[135%]" size={28} color="#0088ca" />
                Ir al Campus
              </Link>
            </SheetClose>
            ...
            {cursosFormacion.map((cursoFormacion, index) => (
              <SheetClose asChild key={index}>
                <Link
                  ...
                  className={`${pathname === `/cursos-formacion/${cursoFormacion.nombre}` && "text-idaclass4"}`}
                >
            ...
            <SheetClose asChild>
              <Link
                href="/comunidad"
                className={`-translate-x-3.5 flex items-center ${pathname.includes("comunidad") && "text-idaclass4"} `}
              >
                <Minus className="rotate-90 -left-3 scale-x-[135%]" size={28} color="#0088ca" />
                Comunidad
              </Link>
            ...
            <SheetClose asChild>
              <Link
                className={`-translate-x-3.5 flex items-center ${pathname === "/nosotros" && "text-idaclass4"} `}
                href="/nosotros"
              >
                <Minus className="rotate-90 -left-3 scale-x-[135%]" size={28} color="#0088ca" />
                Nosotros
